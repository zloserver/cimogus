...
IpcClient *IpcClient::Reinitialize(QObject *parent) {
  ...
  m_instance = new IpcClient(parent);
  if (!m_instance->isSocketConnected()) {
    if (!IpcClient::init(m_instance)) {
...
IpcClient *IpcClient::Instance() { return m_instance; }
...
bool IpcClient::init(IpcClient *instance) {
  ...
  connect(Instance()->m_localSocket.data(), &QLocalSocket::connected,
          &Instance()->m_ClientNode, []() {
            Instance()->m_ClientNode.addClientSideConnection(
                Instance()->m_localSocket.data());

            Instance()->m_ipcClient.reset(
                Instance()->m_ClientNode.acquire<IpcInterfaceReplica>());
            ...
            Instance()->m_Tun2SocksClient.reset(
                Instance()->m_ClientNode.acquire<IpcProcessTun2SocksReplica>());
            ...
            Instance()->m_ipcClient->connectionEstablished(Instance()->m_uuid);
          });
  ...
  qDebug() << "IpcClient::isReplicaValid: "
           << Instance()->m_ipcClient->isReplicaValid()
           << " tun2socks: " << Instance()->m_Tun2SocksClient->isReplicaValid();

  return (Instance()->m_ipcClient->isReplicaValid() &&
          Instance()->m_Tun2SocksClient->isReplicaValid());
}

QSharedPointer<PrivilegedProcess> IpcClient::CreatePrivilegedProcess() {
  if (!Instance()->m_ipcClient || !Instance()->m_ipcClient->isReplicaValid()) {
    qWarning() << "IpcClient::createPrivilegedProcess : IpcClient IpcClient "
                  "replica is not valid";
    return nullptr;
  }

  QRemoteObjectPendingReply<int> futureResult =
      Instance()->m_ipcClient->createPrivilegedProcess();
  ...
  connect(pd->localSocket.data(), &QLocalSocket::connected,
          pd->replicaNode.data(), [pd]() {
            pd->replicaNode->addClientSideConnection(pd->localSocket.data());

            IpcProcessInterfaceReplica *repl =
                pd->replicaNode->acquire<IpcProcessInterfaceReplica>();
            ...
            if (!pd->ipcProcess) {
              ...
              QObject::connect(pd->ipcProcess.data(),
                               &PrivilegedProcess::destroyed,
                               pd->ipcProcess.data(),
